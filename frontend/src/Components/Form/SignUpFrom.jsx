...
export const SignUp = ()=>{
    ...
    return(
        <Form onSubmit={(event)=>{
            ...
            <input type="email" onChange={(event)=>handleChange(event)} placeholder="Email" id="email" className="linput" required/><br/><br/>
            <div id="vdiv">
                ...
                <div className='vcode' style={{width: '25%'}} onClick={()=>{
                    axios.post('https://oppo-backend.herokuapp.com/users/verify',{email:user.email}).then(res=>{
                        setOtp(res.data.otp)
            ...
            <input type="submit" value='Create Account' className="lsubmit vbutton"/>
        </Form>
    )
}